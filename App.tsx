...
export default function App() {
  ...
  const handleSequentialAnalyze = async (trancheId: number, emailsToAnalyze: EnrichedEmail[]) => {
    ...
    try {
      ...
      for (const email of emailsToAnalyze) {
        ...
        setStatusText(`Moteur ${config.provider.toUpperCase()} : ${email.subject?.slice(0, 15)}...`);
        const existingFolders = getExistingFolders();
        
        try {
          // 1. ANALYSE INDIVIDUELLE (Unifiée Puter/Gemini)
          const analysis = await analyzeSingleEmail(email, config.model, config.provider, existingFolders);
          logger.info(`IA OK [${email.subject?.slice(0, 20)}] -> ${analysis.suggestedFolder}`);

          // 2. ACTIONS RÉELLES GMAIL
          setStatusText(`Gmail Sync : ${analysis.suggestedFolder}...`);
      ...
      logger.success("Séquence terminée.");
    } finally {
      setLoading(false);
      setStatusText('');
    }
  ...
  const showFolders = () => {
    ...
    alert(`Dossiers Gmail Actuels :\n\n${folders.join('\n') || 'Aucun dossier détecté'}`);
  };

  const showTags = () => {
    const tags = getExistingTags();
    alert(`Tags Gmail Actuels :\n\n${tags.map(t => `#${t}`).join('\n') || 'Aucun tag détecté'}`);
  };
        ...
        {!userEmail ? (
          <div className="flex flex-col items-center justify-center py-20 text-center px-4">
             <div className="relative group w-full max-w-md">
                ...
                <button onClick={() => (window as any).tokenClient.requestAccessToken({ prompt: 'select_account' })} className="relative w-full py-8 bg-white text-black font-black rounded-[40px] flex items-center justify-center gap-6 hover:scale-[1.02] transition-all active:scale-95 shadow-2xl">
                    ...
                    <span className="text-xl sm:text-2xl tracking-tighter">Sync Gmail</span>
                </button>
             </div>
          </div>
        ) : (
          <div className="animate-in fade-in slide-in-from-bottom-12 duration-1000 space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
               ...
               <button onClick={showFolders} className="p-5 sm:p-6 bg-white/5 border border-white/10 rounded-[32px] font-black text-[10px] sm:text-xs text-white/60 hover:text-white transition-all flex flex-col items-center justify-center gap-2 hover:bg-white/10">
                 <Layers className="w-5 h-5 sm:w-6 sm:h-6" /> Dossiers IA
               </button>
               <button onClick={showTags} className="p-5 sm:p-6 bg-white/5 border border-white/10 rounded-[32px] font-black text-[10px] sm:text-xs text-white/60 hover:text-white transition-all flex flex-col items-center justify-center gap-2 hover:bg-white/10">
            ...
            <div className="bg-white/[0.02] backdrop-blur-3xl p-8 sm:p-12 rounded-[48px] sm:rounded-[56px] border border-white/5 shadow-3xl relative overflow-hidden group">
               <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 relative z-10 gap-6">
                  <div className="flex flex-col">
                    ...
                    <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] mt-2">Emails Organisés avec succès</span>
                  </div>
            ...
            {loading && (
              <div className="fixed bottom-12 inset-x-0 flex justify-center z-[100] pointer-events-none px-4">
                 <div className="bg-white text-black px-8 sm:px-10 py-5 sm:py-7 rounded-[32px] sm:rounded-[40px] shadow-4xl flex items-center gap-6 sm:gap-8 animate-in slide-in-from-bottom-12 pointer-events-auto border-4 border-black">
                    ...
                    <button onClick={() => stopSignal.current = true} className="p-3 bg-red-500 text-white rounded-2xl active:scale-90"><StopCircle className="w-6 h-6 sm:w-7 sm:h-7" /></button>
                 </div>
              </div>
            )}

            <div className="space-y-6 sm:space-y-10">
               <h2 className="text-2xl sm:text-4xl font-black text-white tracking-tighter px-4">Analyse en Séquence</h2>
               {tranches.map(t => (
                 <MissionCard 
    ...
    <div className={`transition-all duration-700 rounded-[40px] sm:rounded-[56px] overflow-hidden border ${isOpen ? 'bg-white/[0.06] border-white/20 shadow-4xl' : 'bg-white/[0.01] border-white/5 hover:bg-white/[0.03]'}`}>
      <div className="flex items-center justify-between p-8 sm:p-12 cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        <div className="flex items-center gap-6 sm:gap-10">
          <div className={`w-14 h-14 sm:w-20 sm:h-20 rounded-[22px] sm:rounded-[28px] flex items-center justify-center transition-all ${tranche.status === 'completed' ? 'bg-emerald-500 text-black shadow-[0_0_30px_rgba(16,185,129,0.3)]' : tranche.status === 'running' ? 'bg-indigo-600 text-white animate-pulse' : 'bg-white/5 text-white/20'}`}>
          ...
          <div>
            <h3 className="text-xl sm:text-3xl font-black text-white tracking-tighter">Bloc de Récolte {tranche.id}</h3>
            <div className="flex items-center gap-3 mt-3">
               ...
               <span className="text-[9px] sm:text-[11px] font-black text-white/30 uppercase tracking-[0.2em]">{tranche.fetchedCount} emails</span>
            </div>
          </div>
        </div>
        <div className="flex items-center">
           {tranche.status !== 'completed' && (
              <button onClick={(e) => { e.stopPropagation(); tranche.status === 'running' ? onStop() : onStart(); }}
                ...
                className={`p-4 sm:p-6 rounded-[20px] transition-all active:scale-90 ${tranche.status === 'running' ? 'bg-red-500 text-white shadow-lg shadow-red-500/20' : 'bg-white text-black hover:scale-105'}`}>
