...
import { EnrichedEmail, HarvestTranche, AIAnalysis, ViewMode, FolderStyle, TrainingStep } from './types';
import { getTotalInboxCount, createGmailLabel, moveEmailsToLabel, applyTagsToEmail, renameAllLabelsToStyle, getUserLabels } from './services/gmailService';
...
export default function App() {
  ...
  const [tranches, setTranches] = useState<HarvestTranche[]>(() => {
    const saved = localStorage.getItem('harvest_tranches_v11');
  ...
  useEffect(() => {
    if (tranches.length > 0) {
      localStorage.setItem('harvest_tranches_v11', JSON.stringify(tranches));
  ...
  const handleSequentialAnalyze = async (trancheId: number, emailsToAnalyze: EnrichedEmail[], retryOnly: boolean = false) => {
    ...
    try {
      ...
      for (let i = 0; i < emailsToProcess.length; i += CONCURRENCY) {
        ...
        setStatusText(`Analyse Titan x${CONCURRENCY} : ${i + chunk.length}/${emailsToProcess.length}`);
  ...
  const startAutopilot = async () => {
    ...
    stopSignal.current = false;
    setAutopilotLogs(["[START] Lancement du mode Autopilote Titanesque.", "[STEP 1] Récolte exhaustive des emails..."]);
    
    // 1. Récolte
    for (const tranche of tranches) {
      if (stopSignal.current) break;
      if (tranche.status !== 'completed') {
        setAutopilotLogs(prev => [`[HARVEST] Début Bloc ${tranche.id}`, ...prev]);
        await runHarvestMission(tranche.id);
      }
    }
    
    if (stopSignal.current) {
      setAutopilotLogs(prev => ["[STOP] Autopilote interrompu par l'utilisateur.", ...prev]);
      setIsAutopilotRunning(false);
      return;
    }

    setAutopilotLogs(prev => ["[STEP 2] Début de l'analyse neuronale...", ...prev]);
    ...
    for (const tranche of tranches) {
      ...
      if (pending.length > 0) {
        setAutopilotLogs(prev => [`[ANALYSE] Traitement Bloc ${tranche.id} (${pending.length} mails)`, ...prev]);
  ...
  const optimizeFolders = async () => {
    ...
    try {
      ...
      const prompt = `Voici ma liste de dossiers Gmail actuels : ${stats.folders.map(f => f[0]).join(', ')}. 
      Suggère-moi des fusions ou des renommages pour simplifier l'organisation. Réponds en JSON : {"suggestions": [{"from": "Ancien", "to": "Nouveau", "reason": "Pourquoi"}]}`;
      ...
      const suggestions = JSON.parse(cleanAIResponse(response.text)).suggestions;
      alert(`Optimisations suggérées : ${suggestions.length}\n${suggestions.map((s:any) => `- ${s.from} -> ${s.to} : ${s.reason}`).join('\n')}`);
    } catch (e) {
  ...
  const startTrainingSession = () => {
    ...
    if (sample.length === 0) return alert("Aucun email pour calibration.");
  ...
  const stats = useMemo(() => {
    ...
    return { folders: Array.from(folders.entries()).sort((a,b) => b[1] - a[1]), tags: Array.from(tags.entries()).sort((a,b) => b[1] - a[1]), count: processed.length };
  }, [allEmails]);
    ...
    <div className="min-h-screen flex flex-col bg-[#050505] text-white overflow-hidden">
        ...
        <nav className="bg-black/80 border-b border-white/5 sticky top-[73px] z-40 backdrop-blur-xl shrink-0">
           <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between gap-4">
              ...
              <div className="relative hidden sm:block w-72">
                ...
                <input type="text" placeholder="RECHERCHE..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl pl-12 pr-4 py-3 text-[10px] font-bold outline-none focus:border-indigo-500 transition-all placeholder:text-white/10" />
              </div>
              <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-3 bg-white/5 rounded-2xl sm:hidden"><Menu className="w-5 h-5" /></button>
           </div>
        </nav>
      )}
        ...
        {isAnalyzing && (
          <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-300">
             <div className="bg-[#0A0A0A] border border-white/10 p-10 rounded-[48px] shadow-4xl flex flex-col items-center gap-6 max-w-sm w-full">
                ...
                <button onClick={() => stopSignal.current = true} className="mt-4 px-6 py-3 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all">Interrompre</button>
             </div>
          </div>
        )}
          ...
          <>
            {viewMode === 'pipeline' && (
              <div className="flex-1 overflow-y-auto p-4 sm:p-10 space-y-10 pb-32 max-w-6xl mx-auto w-full">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                   <button onClick={() => setShowSummary(true)} className="p-8 bg-indigo-600 rounded-[40px] font-black text-[10px] uppercase tracking-widest flex flex-col items-center gap-4 shadow-xl border border-indigo-400"><BarChart3 className="w-8 h-8" /> Bilan</button>
                   ...
                   <button onClick={() => isAutopilotRunning ? (stopSignal.current = true) : startAutopilot()} className={`p-8 rounded-[40px] font-black text-[10px] uppercase tracking-widest flex flex-col items-center gap-4 transition-all ${isAutopilotRunning ? 'bg-red-500 border-red-400 animate-pulse' : 'bg-white/5 border-white/10 hover:bg-white/10'}`}>
                ...
                {isAutopilotRunning && (
                  <div className="bg-indigo-600/10 border border-indigo-500/20 rounded-[48px] p-10 space-y-6">
                     <h3 className="text-xs font-black uppercase tracking-[0.5em] text-indigo-400 flex items-center gap-3"><Zap className="w-5 h-5" /> Autopilote Monitor</h3>
                     <div className="bg-black/40 rounded-3xl p-6 font-mono text-[11px] h-40 overflow-y-auto space-y-2 border border-white/5 no-scrollbar">
                ...
                <div className="space-y-10">
                  <h2 className="text-4xl font-black tracking-tighter px-6">Pipeline Neuronale <span className="text-indigo-400">x{config.concurrency}</span></h2>
                  {tranches.map(t => (
            ...
            {viewMode === 'mail' && (
              <div className="flex-1 flex overflow-hidden">
                <aside className={`${isSidebarOpen ? 'w-80' : 'w-0'} transition-all bg-[#0A0A0A] border-r border-white/5 flex flex-col shrink-0 overflow-hidden`}>
                  <div className="p-8 flex flex-col h-full space-y-10">
                    <button onClick={() => setSelectedFolder(null)} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest ${!selectedFolder ? 'bg-indigo-600 shadow-xl' : 'text-white/40'}`}><Inbox className="w-5 h-5" /> Réception</button>
                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-1">
                       ...
                       {stats.folders.map(([name, count]) => (
                         <button key={name} onClick={() => setSelectedFolder(name)} className={`w-full flex items-center justify-between px-6 py-3 rounded-xl text-[11px] font-bold transition-all ${selectedFolder === name ? 'bg-white/10 text-white' : 'text-white/40 hover:bg-white/5'}`}>
                ...
                <div className="flex-1 flex flex-col overflow-hidden bg-black/20">
                   <div className="p-6 border-b border-white/5 flex justify-between items-center"><button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-3 bg-white/5 rounded-2xl"><Menu className="w-5 h-5" /></button><h3 className="text-xl font-black tracking-tighter">{selectedFolder || "Réception"}</h3><button className="p-3 bg-white/5 rounded-2xl"><RefreshCw className="w-5 h-5" /></button></div>
                   <div className="flex-1 overflow-y-auto divide-y divide-white/5 no-scrollbar">
                     {filteredEmails.map(e => (
                       <div key={e.id} className="flex items-center gap-6 px-8 py-5 hover:bg-white/[0.03] cursor-pointer group transition-all">
                          <Star className="w-4 h-4 text-white/5 group-hover:text-yellow-500/40 shrink-0" />
                          <div className="w-40 sm:w-56 text-[13px] font-black text-white/80 truncate shrink-0">{e.from.split('<')[0]}</div>
                          <div className="flex-1 min-w-0 flex flex-col"><span className="font-bold text-[14px] text-white truncate">{e.subject}</span><span className="text-white/20 text-[12px] truncate">{e.snippet}</span></div>
                          <div className="text-[10px] font-black text-white/10">{new Date(parseInt(e.internalDate)).toLocaleDateString([], {day:'2-digit', month:'2-digit'})}</div>
                       </div>
                     ))}
                   </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-10 max-w-4xl mx-auto w-full">
                {!isTrainingActive ? (
                  <div className="p-20 bg-white/5 rounded-[64px] border border-white/5 flex flex-col items-center gap-10 text-center">
                     ...
                     <button onClick={startTrainingSession} className="px-14 py-7 bg-white text-black rounded-[40px] font-black text-sm">Lancer Session</button>
                  </div>
                ) : (
                  <div className="space-y-6">
                     <div className="flex justify-between items-center mb-10"><h2 className="text-3xl font-black tracking-tighter">Calibration Active</h2><button onClick={() => setIsTrainingActive(false)} className="text-xs font-black text-red-400">Quitter</button></div>
                     {trainingSteps.map((s, i) => (
                       <div key={i} className={`p-10 rounded-[48px] border bg-white/5 border-white/10 ${s.completed ? 'grayscale opacity-30' : ''}`}>
                          <div className="flex flex-col sm:flex-row justify-between items-center gap-8">
                             ...
                             <div className="flex gap-3"><button onClick={() => setTrainingSteps(prev => prev.map((st, idx) => idx === i ? {...st, completed: true} : st))} className="p-6 bg-emerald-500 text-black rounded-3xl"><Check /></button><button className="p-6 bg-white/5 text-white/40 rounded-3xl"><Trash /></button></div>
                          </div>
                       </div>
                     ))}
                  </div>
      ...
      {showSummary && (
        <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-[#080808] w-full max-w-5xl h-[85vh] rounded-[72px] border border-white/10 p-16 overflow-y-auto relative no-scrollbar">
             <button onClick={() => setShowSummary(false)} className="absolute top-12 right-12 p-5 bg-white/5 rounded-full"><X className="w-6 h-6" /></button>
             <div className="space-y-16">
                <h2 className="text-7xl font-black tracking-tighter">Bilan Global</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-20">
                   <div className="space-y-6">
                      <div className="flex justify-between items-center px-2">
                        <h3 className="text-xs font-black uppercase tracking-widest text-white/20">Dossiers Créés</h3>
                        <button onClick={optimizeFolders} className="px-4 py-2 bg-indigo-600 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-indigo-500 transition-all"><Sparkles className="w-3 h-3" /> Optimiser</button>
                      </div>
                   ...
                   <div className="space-y-12">
                      <div className="p-12 bg-indigo-600 rounded-[56px] shadow-3xl flex items-center justify-between">
                         <div><p className="text-8xl font-black">{stats.count}</p><p className="text-[10px] font-black uppercase opacity-60">Emails organisés</p></div>
                         <PieChart className="w-24 h-24 opacity-20" />
                      </div>
                      <div className="space-y-6">
                        <h3 className="text-xs font-black uppercase tracking-widest text-white/20 px-2">Intelligence Tags</h3>
                        <div className="flex flex-wrap gap-3">{stats.tags.map(([n, c]) => (<span key={n} className="px-5 py-2.5 bg-white/5 border border-white/10 rounded-full text-[10px] font-black uppercase tracking-tighter">#{n} <span className="text-indigo-500 ml-2">{c}</span></span>))}</div>
                      </div>
                   </div>
                </div>
             </div>
    ...
    <div className={`transition-all duration-700 rounded-[64px] border ${isOpen ? 'bg-white/[0.06] border-white/20' : 'bg-white/[0.01] border-white/5'}`}>
      <div className="p-10 flex justify-between items-center cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        <div className="flex items-center gap-10">
          <div className={`w-20 h-20 rounded-[32px] flex items-center justify-center ${tranche.status === 'completed' ? 'bg-emerald-500 text-black' : tranche.status === 'running' ? 'bg-indigo-600 animate-pulse' : 'bg-white/5 text-white/20'}`}><Database className="w-10 h-10" /></div>
          <div><h3 className="text-3xl font-black">Bloc {tranche.id}</h3><div className="flex items-center gap-4 mt-3"><div className="w-40 h-2 bg-white/5 rounded-full overflow-hidden"><div className={`h-full transition-all duration-1000 ${tranche.status === 'completed' ? 'bg-emerald-500' : 'bg-indigo-500'}`} style={{ width: `${prog}%` }} /></div><span className="text-[11px] font-black text-white/30">{tranche.fetchedCount} / {tranche.totalToFetch}</span></div></div>
        </div>
        <button onClick={(e) => { e.stopPropagation(); tranche.status === 'running' ? onStop() : onStart(); }} disabled={isLoading && tranche.status !== 'running'} className={`p-6 rounded-[28px] ${tranche.status === 'running' ? 'bg-red-500 text-white' : 'bg-white text-black'}`}>{tranche.status === 'running' ? <StopCircle className="w-8 h-8" /> : <PlayCircle className="w-8 h-8" />}</button>
      </div>
      {isOpen && (
